...
async def test_feedback_persists_in_session(pi_client, tmp_path):
    ...
    for line in lines:
        ...
        if entry.get("type") == "custom" and entry.get("customType") == "pi-portal-feedback":
...
async def test_negative_feedback_clears_comment_on_positive(pi_client, tmp_path):
    ...
    for line in lines:
        ...
        if (entry.get("type") == "custom" and 
            entry.get("customType") == "pi-portal-feedback" and
            entry.get("data", {}).get("targetTimestamp") == target_ts):
            feedback_entries.append(entry)
